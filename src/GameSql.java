...
public class GameSql {
    ...
    public static void add(String username, int score1, String opponent, int score2){
        try{
            // 注册 JDBC 驱动,加载驱动
            Class.forName(JDBC_DRIVER);
            // 打开链接,连接数据库
            ...
            sql = String.format("INSERT INTO userinfo" +
                    "(id,username,game_time,results,opponent,opponent_results) " +
                    "VALUES(%d,'%s','%s',%d,'%s',%d);", userid,username,formattedDateTime, score1,opponent, score2);
    ...
    public static String[][] findscord(String username)
    {
        try{
            ...
            stmt = conn.createStatement();
            String sql;
            sql = "SELECT DISTINCT username, practice_time, practice_results, game_time, results, opponent, opponent_results FROM userinfo WHERE username = '" + username +"';";
            rs = stmt.executeQuery(sql);
            int cnt = 0;
            int rowcnt = 0;
            while(rs.next()) rowcnt ++;
            String[][] data = new String[rowcnt][7];
            rs = stmt.executeQuery(sql);
            while(rs.next())
            {
                ...
                data[cnt][5] = rs.getString("opponent");
                data[cnt][6] = rs.getString("opponent_results");
                cnt ++;
            }
            rs.close();
            stmt.close();
